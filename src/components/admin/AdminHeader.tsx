...
export default function AdminHeader({ onOpenSidebar, user }: AdminHeaderProps) {
  return (
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 text-xs">
            ...
            <div className="hidden md:block text-left">
              <p className="font-semibold text-text-primary leading-tight">{user?.name || 'Administrator'}</p>
              <p className="text-[10px] text-text-tertiary leading-tight">{user?.email || 'admin@kuasampaga.test'}</p>
            </div>
          </div>
        </div>
      </div>
