...
export default function InformationForm({ initialData }: InformationFormProps) {
  ...
  const { undo, redo, canUndo, canRedo, isDirty, markSaved } = useFormHistory(
    {
      title, slug, content, excerpt, images, thumbnail, category, date, published
    },
    (state) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (!initialData && data.data?.id) {
          router.replace(`/admin/informasi-dan-berita/${data.data.id}/edit`);
        } else {
          markSaved({ title, slug, content, excerpt, images, thumbnail, category, date, published });
          router.refresh();
        }
      } else {
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
        ...
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={() => router.push('/admin/informasi-dan-berita')}
