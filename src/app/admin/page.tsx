...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out pb-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-gradient-to-r from-[#064E3B] via-[#047857] to-[#10B981] p-5 sm:p-6 shadow-md">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 bg-white/20 backdrop-blur-md rounded-full text-white text-[10px] font-bold uppercase tracking-wider mb-2 border border-white/20 shadow-sm">
            ...
            <p className="text-emerald-50 mt-1 max-w-xl text-xs sm:text-sm opacity-90 leading-relaxed">
              Ringkasan data, layanan, dan semua aktivitas terbaru di website instansi.
            </p>
          </div>
          <div className="hidden md:flex flex-shrink-0 items-center justify-center w-14 h-14 bg-white/10 rounded-xl backdrop-blur-md border border-white/20 shadow-inner">
        ...
        {stats.map((stat, i) => {
          ...
          return (
            <Link
              key={i}
              href={stat.href}
              className="group relative bg-white rounded-xl border border-gray-100 p-4 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-300 overflow-hidden"
            >
              ...
              <div className="flex items-center gap-3 relative z-10">
                ...
                <div>
                  <span className="block text-xl sm:text-2xl font-black text-gray-800 font-heading tracking-tight">{stat.value}</span>
                  <p className="text-[10px] font-bold text-gray-500 uppercase tracking-wider group-hover:text-gray-800 transition-colors">
                    {stat.label}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm ">
          <div className="flex items-center justify-between mb-5">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-black text-gray-800 font-heading">Status Kapasitas Server</h2>
            </div>
            <DataCleanupModal />
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div className="bg-gray-50/80 p-5 rounded-xl border border-gray-200">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider flex items-center gap-1 border ${dbUsage.isDanger ? 'bg-red-50 text-red-700 border-red-200' : dbUsage.isWarning ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="relative w-24 h-24 flex-shrink-0">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    {/* Progress Circle */}
                    <path
                      className={`${dbUsage.isDanger ? 'text-red-500' : dbUsage.isWarning ? 'text-yellow-400' : 'text-emerald-500'} transition-all duration-1000 ease-out`}
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-sm font-black text-gray-800">{dbUsage.percentage.toFixed(1)}%</span>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 flex items-center justify-between">
                      <span>Total Kapasitas</span>
                      <span className="text-[11px] font-black text-gray-800">{dbUsage.maxCapacityMB} MB</span>
                    </p>
                    <div className="w-full border-b border-gray-200 border-dashed mt-1"></div>
                  </div>
                  <div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 flex items-center justify-between">
            ...
            <div className="bg-gray-50/80 p-5 rounded-xl border border-gray-200">
              <div className="flex items-center justify-between mb-6">
                ...
                <span className={`text-[9px] font-black px-2 py-0.5 rounded-md uppercase tracking-wider flex items-center gap-1 border ${storageUsage.isDanger ? 'bg-red-50 text-red-700 border-red-200' : storageUsage.isWarning ? 'bg-yellow-50 text-yellow-700 border-yellow-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="relative w-24 h-24 flex-shrink-0">
                  <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                    ...
                    {/* Progress Circle */}
                    <path
                      className={`${storageUsage.isDanger ? 'text-red-500' : storageUsage.isWarning ? 'text-yellow-400' : 'text-amber-500'} transition-all duration-1000 ease-out`}
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="text-sm font-black text-gray-800">{storageUsage.percentage.toFixed(1)}%</span>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 flex items-center justify-between">
                      <span>Total Kapasitas</span>
                      <span className="text-[11px] font-black text-gray-800">{storageUsage.maxCapacityMB >= 1024 ? `${(storageUsage.maxCapacityMB / 1024).toFixed(1)} GB` : `${storageUsage.maxCapacityMB} MB`}</span>
                    </p>
                    <div className="w-full border-b border-gray-200 border-dashed mt-1"></div>
                  </div>
                  <div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 flex items-center justify-between">
                      <span className="flex items-center gap-1.5"><span className="inline-block w-1.5 h-1.5 rounded-full bg-amber-500"></span> Terpakai</span>
                      <span className="text-[11px] font-black text-amber-600">{storageUsage.sizeInMB} MB</span>
                    </p>
                  </div>
                  <div>
                    <p className="text-[9px] text-gray-500 font-bold uppercase tracking-wider mb-0.5 flex items-center justify-between">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
        <h2 className="text-xs font-black text-gray-800 uppercase tracking-widest mb-3 font-heading flex items-center gap-1.5">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          >
            <div className="bg-white/20 p-1.5 rounded-md"><Plus className="w-3.5 h-3.5" /></div>
            Layanan Baru
          </Link>
          <Link
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-black text-gray-800 font-heading flex items-center gap-2">
              <div className="w-7 h-7 rounded-lg bg-blue-50 text-blue-600 flex items-center justify-center">
                <FileText className="w-3.5 h-3.5" />
              </div>
              Layanan Terbaru
            </h2>
            <Link href="/admin/layanan" className="px-3 py-1.5 bg-gray-50 hover:bg-gray-100 rounded-lg text-[10px] font-bold text-gray-600 hover:text-blue-600 transition-colors flex items-center gap-1">
              Lihat Semua <ArrowRight className="w-3 h-3" />
            </Link>
          </div>
          <div className="space-y-2">
            {recentServices.map(service => (
              <div key={service.id} className="group flex items-center justify-between p-2.5 rounded-lg bg-gray-50/50 hover:bg-blue-50/50 border border-transparent hover:border-blue-100 transition-all">
                <div>
                  <p className="text-xs font-bold text-gray-800 group-hover:text-blue-700 transition-colors">{service.title}</p>
                  <p className="text-[9px] font-semibold text-gray-500 mt-0.5 uppercase tracking-wider">{service.category} · {service.requirements.length} Persyaratan</p>
                </div>
                <Link
                  href={`/admin/layanan/${service.id}/edit`}
                  className="w-6 h-6 rounded-md bg-white border border-gray-200 flex items-center justify-center text-gray-400 hover:text-blue-600 hover:border-blue-300 hover:shadow-sm transition-all"
                >
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-4 shadow-sm">
          ...
          {recentLogs.length > 0 ? (
            <div className="space-y-3">
              {recentLogs.map(log => (
                <div key={log.id} className="flex items-start gap-2.5 pb-3 border-b border-gray-100 last:border-0 last:pb-0">
                  <div className="w-1.5 h-1.5 rounded-full bg-purple-500 mt-1.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-[11px] font-black text-gray-800">{log.action}</p>
            ...
            <p className="text-xs font-semibold text-gray-400 py-6 text-center bg-gray-50 rounded-lg border border-dashed border-gray-200">Belum ada aktivitas tercatat.</p>
          )}
        </div>
