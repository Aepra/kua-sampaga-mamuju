  ...
  return (
    <div className="min-h-screen bg-[#F8FAF9] dark:bg-gray-900">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Search and filters */}
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-[#E5EBE5] dark:border-gray-700 shadow-sm p-5 mb-8">
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6B7E6B] dark:text-gray-400" />
              <input
                type="text"
                value={query}
                onChange={e => setQuery(e.target.value)}
                placeholder="Cari layanan..."
                className="w-full pl-10 pr-4 py-2.5 text-sm bg-[#F8FAF9] dark:bg-gray-900 border border-[#E5EBE5] dark:border-gray-700 rounded-xl focus:border-[#10B981] dark:focus:border-emerald-500 focus:ring-1 focus:ring-[#10B981] dark:focus:ring-emerald-500/20 outline-none transition-all placeholder-[#6B7E6B]"
              />
            </div>
            <div className="flex items-center gap-2">
              <div className="relative">
                <SlidersHorizontal className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6B7E6B] dark:text-gray-400 pointer-events-none" />
                <select
                  value={sortBy}
                  onChange={e => setSortBy(e.target.value as 'name' | 'category')}
                  className="pl-10 pr-8 text-sm bg-[#F8FAF9] dark:bg-gray-900 border border-[#E5EBE5] dark:border-gray-700 rounded-xl py-2.5 outline-none focus:border-[#10B981] dark:focus:border-emerald-500 focus:ring-1 focus:ring-[#10B981] dark:focus:ring-emerald-500/20 text-[#4A5D4A] dark:text-gray-300 appearance-none"
                >
                  <option value="name">Nama A-Z</option>
                  <option value="category">Kategori</option>
                </select>
                <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-[#6B7E6B] dark:text-gray-400">
        ...
        {/* Results */}
        <p className="text-sm text-[#6B7E6B] dark:text-gray-400 font-medium mb-4">
          Ditemukan <span className="text-[#059669] dark:text-emerald-400 font-bold">{filtered.length}</span> layanan
        </p>
